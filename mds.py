...
def test( ):
    ...
    for index in range(len(Beacon1List)):
        points[[index, Beacon1List[index]]] = points[[Beacon1List[index] ,index]]
        print("points[{}, {}] = {}".format(index, Beacon1List[index], points[[index, Beacon1List[index]]]))
    print('points now is ', points)
    ...
    print(abs_map)
    rmsd = (1 / len(points) * np.sum(
        np.sqrt((abs_map[:, 0] - points[:, 0]) ** 2 + (abs_map[:, 1] - points[:, 1]) ** 2))) ** 0.5
    print('rmsd is = ', rmsd)
